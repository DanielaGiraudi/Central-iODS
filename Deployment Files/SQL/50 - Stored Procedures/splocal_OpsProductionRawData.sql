...
IF EXISTS (SELECT 1 FROM Information_schema.Routines 
				...
				AND specific_name = 'splocal_OpsProductionRawData' 
				AND Routine_Type = 'PROCEDURE')

BEGIN
	DROP PROCEDURE [dbo].[splocal_OpsProductionRawData]
END
GO

------------------------------------------------------------------------------------------------------------------------
-- 												OPS Database Script													--	
--				This Script will create the splocal_OpsProductionRawData stored procedure in OpsDataStore			--
--										SPROC to get Row Data from Production.										--
------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[splocal_OpsProductionRawData]  
--DECLARE
		@StartTime			DATETIME , 
		@EndTime			DATETIME , 
		@Site				NVARCHAR(MAX),
	...
	Set @strQuery = N'
		SELECT '+ @Columns +' FROM (
			SELECT ROW_NUMBER() OVER(ORDER BY '+ CASE -- ORDERING
													...
													END 
			+') AS RowNum, opsPD.*
			FROM [dbo].[OpsDB_Production_Data] opsPD WITH(NOLOCK) ' +
			CASE WHEN (ISNULL(@Units,'') = '') THEN 'JOIN [dbo].[WorkCell_Dimension]		w	WITH(NOLOCK) ON opsPD.PUID = w.PUID AND w.Class like ''1''' ELSE '' END +
			...
			+ ISNULL(CASE WHEN @is_id_unit = 1 AND @Units IS NOT NULL AND @Units <> '' THEN ' AND opsPD.PUID in ('+ @Units +')' ELSE ' AND ( opsPD.PUDesc in ('+ @Units +') OR opsPD.PUDescGlobal in ('+ @Units +') )' END, '')
		+ ') s ' + CASE -- PAGINATION
...
GRANT EXECUTE ON OBJECT ::[dbo].[splocal_OpsProductionRawData] TO [LocalUser];
GO