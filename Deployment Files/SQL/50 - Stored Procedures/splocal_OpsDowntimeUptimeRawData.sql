...
IF EXISTS (SELECT 1 FROM Information_schema.Routines 
				...
				AND specific_name = 'splocal_OpsDowntimeUptimeRawData' 
				AND Routine_Type = 'PROCEDURE')

BEGIN
	DROP PROCEDURE [dbo].[splocal_OpsDowntimeUptimeRawData]
END
GO

------------------------------------------------------------------------------------------------------------------------
-- 												OPS Database Script													--	
--				This Script will create the splocal_OpsDowntimeUptimeRawData stored procedure in OpsDataStore			--
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[splocal_OpsDowntimeUptimeRawData]  
--DECLARE
		@StartTime			DATETIME , 
		@EndTime			DATETIME , 
		@Site				NVARCHAR(MAX),
	...
	Set @strQuery = N'
		SELECT '+ @Columns +' FROM (
			SELECT ROW_NUMBER() OVER(ORDER BY '+ CASE -- ORDERING
													...
													END 
			+') AS RowNum, opsPD.*
			FROM [dbo].[OpsDB_DowntimeUptime_Data] opsPD WITH(NOLOCK) ' +
...
GRANT EXECUTE ON OBJECT ::[dbo].[splocal_OpsDowntimeUptimeRawData] TO [LocalUser];
GO