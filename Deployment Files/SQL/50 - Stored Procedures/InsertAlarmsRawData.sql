...
AS
	...
	SELECT
		...
		JOIN dbo.LINE_DIMENSION ld ON al.PLDesc = ld.LineDesc AND ld.SITE_DIMENSION_SiteId = @SiteId
		WHERE NOT EXISTS (  SELECT * 
							FROM [dbo].[OpsDB_Alarms_RawData] u 
							WHERE
							u.RCDIDX				= al.RcdIdx AND 
...
JOIN (
				...
				ld.CentralLineId							,
				RcdIdx	
                FROM @AlarmsRawdata d 
				JOIN dbo.LINE_DIMENSION ld ON d.PLDesc= ld.LineDesc AND ld.SITE_DIMENSION_SiteId = @SiteId
) S ON (
