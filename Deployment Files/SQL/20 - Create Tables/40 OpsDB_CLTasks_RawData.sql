...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[OpsDB_CLTasks_RawData]') AND type in (N'U'))
BEGIN
CREATE TABLE [dbo].[OpsDB_CLTasks_RawData] (
...
CONSTRAINT [FK_OpsDB_CLTasks_RawData_FACT_UDPs]
	FOREIGN KEY ([FACT_UDPs_Idx])
	REFERENCES [dbo].[FACT_UDPs] ([CentralIdx]),
CONSTRAINT [FK_OpsDB_CLTasks_RawData_LINE_DIMENSION]
	FOREIGN KEY ([LINE_DIMENSION_CentralLineId])
	REFERENCES [dbo].[LINE_DIMENSION] ([CentralLineId]),
CONSTRAINT [FK_OpsDB_CLTasks_RawData_SITE_DIMENSION]
	FOREIGN KEY ([SITE_DIMENSION_SiteId])
	REFERENCES [dbo].[SITE_DIMENSION] ([SiteId]),
CONSTRAINT [PK_OpsDB_CLTasks_RawData]
	PRIMARY KEY NONCLUSTERED ([CentralRcdIdx])
END
GO

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[OpsDB_CLTasks_RawData]') AND name = N'IDX_UQ_OpsDB_CLTasks_RawData_SiteId_RcdIdx')
	CREATE UNIQUE NONCLUSTERED INDEX [IDX_UQ_OpsDB_CLTasks_RawData_SiteId_RcdIdx] ON [dbo].[OpsDB_CLTasks_RawData]
